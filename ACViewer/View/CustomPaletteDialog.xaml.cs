...
namespace ACViewer.View
{
    public partial class CustomPaletteDialog : UserControl
    {
        ...
        private static bool _textureThumbsWarmed; private const int TextureWarmLimit = 300;

        // Loading overlay elements
        private Grid _rootGrid;
        private Border _loadingOverlay;
        private TextBlock _loadingText;
        private TabItem _texturesTab;
        ...
        { private uint _newId; public int PartIndex { get; set; } public uint OldId { get; set; } public uint NewId { get => _newId; set { if (_newId != value) { _newId = value; OnPropertyChanged(nameof(NewId)); OnPropertyChanged(nameof(NewHex)); } } } public bool IsLocked { get; set; } public string OldHex => $"0x{OldId:X8}"; public string NewHex => $"0x{NewId:X8}"; public event PropertyChangedEventHandler PropertyChanged; private void OnPropertyChanged(string n) => PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(n)); }
        ...
        private readonly SemaphoreSlim _thumbSemaphore = new(4);
        ...
        private UIElement BuildOriginalUI()
        {
            ...
            gridTextures = new DataGrid { AutoGenerateColumns = false, CanUserAddRows = false, Margin = new Thickness(0,0,6,0), IsReadOnly = false, ItemsSource = _textureRowsObs, SelectionMode = DataGridSelectionMode.Extended, SelectionUnit = DataGridSelectionUnit.FullRow, EnableRowVirtualization = true };
            ...
            try
            {
                ...
                if (indexed)
                {
                    ...
                    if (decoded != null)
                    {
                        ...
                        try { decoded.GetData(cols); } catch { }
        ...
        private ImageSource GetOrQueueThumbnail(uint id, System.Windows.Controls.Image target){ if (_thumbCache.TryGetValue(id, out var cached) && cached != null) return cached; _thumbPlaceholder ??= CreatePlaceholder(); var placeholder = _thumbPlaceholder; lock (_thumbSync){ if (_thumbInFlight.Contains(id)) return placeholder; _thumbInFlight.Add(id); } _ = Task.Run(async () => { try { await _thumbSemaphore.WaitAsync().ConfigureAwait(false); ImageSource src=null; try { src=BuildTexturePreview(id);} catch {} if (src!=null){ _thumbCache[id]=src; try { _=Dispatcher.BeginInvoke(new Action(()=>{ if (target!=null) target.Source=src; }), DispatcherPriority.Background);} catch { } } } finally { _thumbSemaphore.Release(); lock(_thumbSync){ _thumbInFlight.Remove(id);} } }); return placeholder; }
        ...
        private Stack<Dictionary<(int part,uint oldId),uint>> _textureUndo=new(); private Stack<Dictionary<(int part,uint oldId),uint>> _textureRedo=new(); private bool _suppressTextureUndo; private Dictionary<(int part,uint oldId),uint> SnapshotTextureMapping()=>_textureRowsObs.ToDictionary(r=>(r.PartIndex,r.OldId),r=>r.NewId); private void PushTextureSnapshot(){ if(_suppressTextureUndo) return; _textureUndo.Push(SnapshotTextureMapping()); _textureRedo.Clear(); } private void ApplyTextureMapping(Dictionary<(int part,uint oldId),uint> map){ if(map==null) return; _suppressTextureUndo=true; foreach(var row in _textureRowsObs) if(map.TryGetValue((row.PartIndex,row.OldId), out var newId) && !row.IsLocked) row.NewId=newId; _suppressTextureUndo=false; ScheduleTextureApply(); SaveLocalTextureOverrides(); }
        private void BtnTexUndo_Click(object sender, RoutedEventArgs e)=>BtnTexUndo(); private void BtnTexRedo_Click(object sender, RoutedEventArgs e)=>BtnTexRedo(); private void BtnTexUndo(){ if(_textureUndo.Count==0) return; var current=SnapshotTextureMapping(); var prev=_textureUndo.Pop(); _textureRedo.Push(current); ApplyTextureMapping(prev); } private void BtnTexRedo(){ if(_textureRedo.Count==0) return; var current=SnapshotTextureMapping(); var next=_textureRedo.Pop(); _textureUndo.Push(current); ApplyTextureMapping(next); }
        private void BtnTexApply_Click(object sender,RoutedEventArgs e)=>ScheduleTextureApply(); private void BtnTexResetSelected_Click(object sender,RoutedEventArgs e){ var sel=gridTextures?.SelectedItems?.Cast<TextureRow>().ToList(); if(sel==null||sel.Count==0) return; PushTextureSnapshot(); foreach(var r in sel) if(!r.IsLocked) r.NewId=r.OldId; ScheduleTextureApply(); SaveLocalTextureOverrides(); }
        ...
        private async Task PrewarmTextureThumbnailsAsync()
        {
            if (_textureThumbsWarmed) return;
            _textureThumbsWarmed = true;
            if (DatManager.PortalDat == null || _availableTextureIds == null || _availableTextureIds.Count == 0)
                return;
            var ids = _availableTextureIds.Take(TextureWarmLimit).ToList();
            try
            {
                await Task.Run(() =>
                {
                    foreach (var id in ids)
                    {
                        try
                        {
                            var img = BuildTexturePreview(id);
                            if (img != null)
                                _thumbCache.TryAdd(id, img);
                        }
                        catch { }
                    }
                }).ConfigureAwait(false);
            ...
            try { Dispatcher.BeginInvoke(new Action(RefreshTextureGallery), DispatcherPriority.Background); } catch { }
        }
    }
}
