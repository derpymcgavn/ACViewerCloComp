...
namespace ACViewer
{
    public static class FileExport
    {
        public static bool ExportRaw(DatType datType, uint fileID, string outFilename)
        {
            ...
            switch (datType)
            {
                case DatType.Cell: datDatabase = DatManager.CellDat; break;
                case DatType.Portal: datDatabase = DatManager.PortalDat; break;
                case DatType.HighRes: datDatabase = DatManager.HighResDat; break;
                case DatType.Language: datDatabase = DatManager.LanguageDat; break;
            }
            ...
            MainWindow.StatusSink?.Post($"Wrote {outFilename}");
            return true;
        }
        ...
        public static bool ExportModel(uint fileID, string outFilename)
        {
            ...
            var startIdx = 0; var startUVIdx = 0;
            if (isSetup)
            {
                ...
                for (var i = 0; i < setup.Parts.Count; i++)
                {
                    ...
                    if (part == 0x010001ec) continue; // skip anchors
            ...
            System.IO.File.WriteAllText(outFilename, sb.ToString());
            MainWindow.StatusSink?.Post($"Wrote {outFilename}");
        ...
        private static void ExportGfxObj(uint gfxObjID, StringBuilder sb, ref int startIdx, ref int startUVIdx, Matrix4x4 transform, Dictionary<uint, bool> surfaceIDs)
        {
            ...
            foreach (var poly in gfxObj.Polygons.OrderBy(i => i.Key).Select(i => i.Value))
            {
                ...
                for (var i = 0; i < poly.VertexIds.Count; i++)
                {
                    ...
                    var uvIdx = vertexUVs[new VertexUV(v, i < poly.PosUVIndices.Count ? poly.PosUVIndices[i] : 0)];
            ...
            foreach (var surfaceID in gfxObj.Surfaces)
            {
                ...
                else surfaceIDs.Add(surfaceID, _gfxObj.HasWrappingUVs);
            }
        }
        ...
        private static void ExportSurfaces(uint fileID, Dictionary<uint, bool> surfaceIDs, string outFilename)
        {
            ...
            foreach (var kvp in surfaceIDs)
            {
                ...
                var surfaceFilename = fi.DirectoryName + System.IO.Path.DirectorySeparatorChar + $"{surfaceID:X8}.png";
                if (!System.IO.File.Exists(surfaceFilename))
                    ExportImage(surfaceID, surfaceFilename);
                sb.AppendLine();
                sb.AppendLine($"newmtl {surfaceID:X8}");
                sb.AppendLine($"map_Kd {surfaceID:X8}.png");
            }
            System.IO.File.WriteAllText(outFilename, sb.ToString());
        ...
        public static bool ExportImage(uint fileID, string outFilename)
        {
            ...
            if (fileType == 0x8)
            {
                ...
                fileID = surface.OrigTextureId; fileType = 0x05;
            }
            Bitmap highRes = null;
            if (fileType == 0x5)
            {
        ...
        public static bool ExportModel_Assimp(uint fileID, MotionData motionData, string outFilename)
        {
            ...
            var scene = new Assimp.Scene { RootNode = new Assimp.Node() };
            var isSetup = fileID >> 24 == 0x2;
            ...
            if (isSetup)
            {
                ...
                for (var i = 0; i < setup.Parts.Count; i++)
                {
                    ...
                    if (placementFrames != null && i < placementFrames.Count)
                    {
                        var pf = placementFrames[i];
                        var rotate = new System.Numerics.Quaternion(-pf.Orientation.X, pf.Orientation.Z, pf.Orientation.Y, pf.Orientation.W);
                        var translate = new Vector3(-pf.Origin.X, pf.Origin.Z, pf.Origin.Y);
                        transform *= Matrix4x4.CreateFromQuaternion(rotate) * Matrix4x4.CreateTranslation(translate);
                    }
                    if (transform != Matrix4x4.Identity)
                        meshNode.Transform = new Assimp.Matrix4x4(transform.M11, transform.M21, transform.M31, transform.M41, transform.M12, transform.M22, transform.M32, transform.M42, transform.M13, transform.M23, transform.M33, transform.M43, transform.M14, transform.M24, transform.M34, transform.M44);
                    var meshes = ExportGfxObj_Assimp(gfxObjId, materialIdx);
                    foreach (var mesh in meshes) { scene.Meshes.Add(mesh); meshNode.MeshIndices.Add(scene.Meshes.Count - 1); }
            ...
            if (motionData != null) BuildAnimation_Assimp(scene, motionData);
            using (var ctx = new AssimpContext())
            {
                if (outFilename.EndsWith(".fbx"))
                {
                    if (ctx.ExportFile(scene, outFilename, "fbx")) View.MainWindow.StatusSink?.Post($"Wrote {outFilename}");
                    else View.MainWindow.StatusSink?.Post($"Failed to export {outFilename}", Services.StatusSeverity.Error);
                    FixFBX(outFilename);
                }
                else if (outFilename.EndsWith(".dae"))
                {
                    if (ctx.ExportFile(scene, outFilename, "collada")) View.MainWindow.StatusSink?.Post($"Wrote {outFilename}");
                    else View.MainWindow.StatusSink?.Post($"Failed to export {outFilename}", Services.StatusSeverity.Error);
                }
                else return false;
            }
